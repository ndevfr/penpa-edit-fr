...
function add_constraints() {
  ...
  penpa_constraints["options_groups"].forEach(function (element, index) {
    ...
    penpa_constraints["options"][element].forEach(function (
      subelement,
      subindex
    ) {
...
function set_answer_setting_table_to(and_or) {
  ...
  [...ands].forEach((elem) =>
    elem.setAttribute("style", `display: ${display[0]};`)
  );
  [...ors].forEach((elem) =>
    elem.setAttribute("style", `display: ${display[1]};`)
  );
}
...
function create_newboard() {
  ...
  if (12 <= size && size <= 90) {
    ...
    if (
      !(gridtype === "square" || gridtype === "sudoku" || gridtype === "kakuro")
    ) {
...
function make_class(gridtype, loadtype = "new") {
  ...
  switch (gridtype) {
    case "square":
      ...
      document.getElementById("nb_sudoku3_lb").innerHTML =
      ...
      if (
        ...
        errorMsg(
          'Rows/Columns Size must be in the range <h2 class="warn">1-' +
            gridmax["square"] +
            "</h2>"
        );
      }
    ...
    case "hex":
      ...
      document.getElementById("nb_sudoku3_lb").style.display = "inline";
      document.getElementById("nb_sudoku3_lb").innerHTML =
        "*White space is subtracted from the Side size";
      if (n0 <= gridmax["hex"] && n0 > 0 && space1 < n0) {
        pu = new Puzzle_hex(n0, n0, size);
      } else {
        errorMsg(
          'Side Size must be in the range <h2 class="warn">1-' +
            gridmax["hex"] +
            "</h2>"
        );
      }
    ...
    case "sudoku":
      if (loadtype === "new") {
        if (document.getElementById("nb_sudoku2").checked === true) {
          // Outside, little killer
          if (document.getElementById("nb_sudoku6").checked === true) {
            // 8x8 grid
            var nx = 10;
            var ny = 10;
          } else if (document.getElementById("nb_sudoku5").checked === true) {
            // 6x6 grid
            var nx = 8;
            var ny = 8;
          } else if (document.getElementById("nb_sudoku8").checked === true) {
            // 4x4 grid
            var nx = 6;
            var ny = 6;
          } else {
        ...
        } else if (document.getElementById("nb_sudoku3").checked === true) {
          // sandwich
          if (document.getElementById("nb_sudoku6").checked === true) {
            // 8x8 grid
            var nx = 9;
      ...
      if (loadtype === "new") {
        ...
        if (document.getElementById("nb_sudoku2").checked === true) {
          ...
          if (document.getElementById("nb_sudoku1").checked === true) {
            // Top left to bottom right diagonal
            linestyle = 12;
            pu.draw_N(start, end, linestyle);
          }

          if (document.getElementById("nb_sudoku4").checked === true) {
...
function changetype() {
  ...
  switch (UserSettings.gridtype) {
    ...
    case "sudoku":
      ...
      document.getElementById("nb_sudoku7_lb").innerHTML =
        "*Default size is 9x9";
    ...
    case "truncated_square":
      ...
      document.getElementById("nb_sudoku3_lb").style.display = "inline";
      document.getElementById("nb_sudoku3_lb").innerHTML =
        "<span style='color: red;'>**Alpha Version - It's under development and currently has limited functionality</span>";
...
function CreateCheck() {
  Swal.fire({
    title:
      'Are you sure want to reset the current board? To only change display size and grid lines use "Update display" button',
...
function replay_choice() {
  if (document.getElementById("replay_choice").value == "2") {
    ...
    if (
      pu.puzzleinfo.totalMS <= pu.replaycutoff &&
      ((redo_len > 0 &&
        typeof pu[pu.mode.qa]["command_redo"].__a[redo_len - 1][5] !=
          "undefined") ||
        (undo_len > 0 &&
          typeof pu[pu.mode.qa]["command_undo"].__a[undo_len - 1][5] !=
            "undefined"))
    ) {
      ...
      pu.live_replay = function () {
        ...
        if (undo_len === 0 && pu.first_click) {
          ...
          let speed_factor = parseFloat(
            document.getElementById("replay_speed").value
          );
...
function panel_onoff() {
  if (UserSettings.panel_shown) {
    ...
    if (window.panel_toplast && window.panel_leftlast) {
      document.getElementById("float-key-body").style.left =
        window.panel_leftlast;
      document.getElementById("float-key-body").style.top =
        window.panel_toplast;
      document.getElementById("float-key-header").style.left =
        window.panel_leftlast;
      document.getElementById("float-key-header").style.top =
        window.panel_toplast;
    } else {
    ...
    let mode_loc = penpa_modes["square"]["mode"].indexOf(
      pu.mode[pu.mode.qa].edit_mode
    );
    document.getElementById("float-key-header-lb").innerHTML =
      "Mode: " + modes_mapping[mode_loc];
...
function advancecontrol_off(loadtype) {
  ...
  var user_choices =
    loadtype === "url" ? UserSettings.tab_settings : getValues("mode_choices");

  if (document.getElementById("tab-dropdown-lite-btn")) {
    document.getElementById("tab-dropdown-lite-btn").innerText =
      "Disable Penpa Lite";
  }
  ...
  if (
    user_choices.indexOf("Line Normal") === -1 &&
  ...
  if (
    user_choices.indexOf("Edge Normal") === -1 &&
  ...
  if (
    user_choices.indexOf("Number Normal") === -1 &&
  ...
  if (
    user_choices.indexOf("Special") === -1 &&
  ...
  if (
    user_choices.indexOf("Sudoku Normal") === -1 &&
...
function ResetCheck() {
  if (pu.mode[pu.mode.qa].edit_mode.toUpperCase() === "LINE") {
    ...
    Swal.fire({
      title:
        "Erase/Clear all " +
        pu.mode[pu.mode.qa].edit_mode.toUpperCase() +
        " mode elements?",
      html: '<h4 class="warn">You won\'t be able to revert this!</h4>',
      icon: "warning",
      showCancelButton: true,
...
function saveimage_download() {
  ...
  if (valid_name) {
    ...
    errorMsg(
      'The characters <h2 class="warn">\\ / : * ? " < > |</h2> cannot be used in filename'
    );
  }
}
...
function io_sudoku() {
  ...
  document.getElementById("iostring").placeholder =
    "Enter digits (0-9, 0 or . for an empty cell, no spaces). The number of digits entered should be a perfect square. Default expected length is 81 digits (9x9 sudoku)";
  document.getElementById("iostring").focus();
}

function i_url() {
  document.getElementById("modal-load").style.display = "block";
  document.getElementById("urlstring").placeholder =
    'In case of "URL too long Error". Type/Paste Penpa-edit URL here and click on Load button. You can also load puzz.link puzzles here';
  ...
  if (
    ...
    if (size <= pu.nx) {
      if (size > 0 && size < 10) {
        ...
        document.getElementById("iostring").value =
          "Error: Min/Max Sudoku Size allowed is 1x1 to 9x9 (Default is 9x9). Update the input parameters below.";
      }
    } else {
      document.getElementById("iostring").value =
        "Error: Grid size is smaller than the specified Sudoku size (Default is 9x9). Update the input parameters below.";
    }
  } else {
    document.getElementById("iostring").value =
      "Error: The canvas area should be a sudoku grid or square grid";
  }
}
...
function load(urlParam, type = "url", origurl = null) {
  ...
  if (pu.user_tags.length === 0) {
    ...
    if (title_words) {
      ...
      if (sudoku_index === 0) {
        ...
        title_words[0].toLowerCase() === "star" &&
        title_words[1].toLowerCase() === "battle"
      ) {
        ...
        title_words[0].toLowerCase() === "spiral" &&
        title_words[1].toLowerCase() === "galaxies"
      ) {
    ...
    if (paramArray.l === "solvedup") {
      ...
      if (rtext[7] !== "undefined") {
        ...
        if (starttime.length === 4) {
          ...
          // added "days" precision in the recent update
          sw_timer.stop(); // stop previously running timer and start with stored starting time
          sw_timer.start({
            precision: "secondTenths",
            startValues: {
      ...
      if (rtext[8]) {
        ...
        var settingstatus = document
          .getElementById("answersetting")
          .getElementsByClassName("solcheck");
      ...
      if (rtext[9] && rtext[9].indexOf("comp") !== -1) {
        // Competitive mode
        set_contestmode();
      }
    } else {
      ...
      if (
        ...
        if (type === "local") {
          ...
          document.getElementById("constraints_settings_opt").style.display =
            "none";
        }
      }
    }
  ...
  if (paramArray.m === "solve") {
    ...
    if (rtext_mode[1]) {
      ...
      if (amode != "board" && amode != "cage" && amode != "special") {
        // Excluding the mode which are not part of answer mode
  ...
  if (rtext[16] && rtext[16] !== "") {
    // for some reason old links had 16th entry as empty
    // set the answer check settings
    var settingstatus = document
      .getElementById("answersetting")
      .getElementsByClassName("solcheck_or");
  ...
  if (
    !valid_replay &&
    (paramArray.m === "solve" || paramArray.l === "solvedup") &&
    type != "localstorage"
  ) {
  ...
  if (valid_replay) {
    ...
    let contents_choice =
      `<select name ="replay_choice" id ="replay_choice" class="replay">` +
    ...
    let contents_speed =
      `<select name ="replay_speed" id ="replay_speed" class="replay">` +
    ...
    if (paramArray.q) {
      ...
      if (document.getElementById("saveinfoauthor").value) {
        disptext +=
          "Author: " + document.getElementById("saveinfoauthor").value + " | ";
      }
      if (qstr.sname) {
      ...
      pu.puzzleinfo.totalMS =
        (+solvetime[0] * 24 * 60 * 60 +
          +solvetime[1] * 60 * 60 +
          +solvetime[2] * 60 +
          +solvetime[3] +
          +solvetime[4] * 0.1) *
...
function set_contestmode() {
  ...
  document.getElementById("answer_key").innerHTML =
    "*Note the Solution Code, go back to <a href=" +
    document.getElementById("saveinfosource").value +
    ' target="_blank">Source</a> and enter in the Submissions Box*';
  pu.undoredo_disable = true;
  pu.comp = true;
}
...
function display_answercheck() {
  ...
  if (answer_check_opt.answercheck_opt.length === 0) {
    infoMsg(
      'No specific option selected by Author. Answer check looks for all the elements with appropriate accepted colors. Check <a href="https://github.com/swaroopg92/penpa-edit/blob/master/images/multisolution.PNG" target="_blank">this</a> for reference.'
    );
  } else {
...
function decode_puzzlink(url) {
  ...
  if (cols > 65 || rows > 65) {
    errorMsg(
      "Penpa+ does not support grid size greater than 65 rows or columns"
    );
    return;
  }
  ...
  switch (type) {
    ...
    case "sudoku":
      ...
      if (cols === 9 && rows === 9) {
        ...
        errorMsg(
          `Sorry, sudoku grids of size: ${cols}x${rows} are not supported`
        );
        break;
      }
    ...
    case "stostone":
      ...
      info_edge = puzzlink_pu.decodeBorder();
      info_number = puzzlink_pu.decodeNumber16();
      info_number = puzzlink_pu.moveNumbersToRegionCorners(
        info_edge,
        info_number
      );
    ...
    case "nonogram":
      ...
      for (var i in info_number) {
        if (i < max_rows_offset * cols) {
          rows_offset = Math.max(
            rows_offset,
            parseInt(i % max_rows_offset) + 1
          );
        } else {
          cols_offset = Math.max(
            cols_offset,
            parseInt((i - max_rows_offset * cols) % max_cols_offset) + 1
          );
        }
      }
      ...
      for (i in info_number) {
        if (i < max_rows_offset * cols) {
          ...
          row_ind =
            rows_offset +
            parseInt((i - max_rows_offset * cols) / max_cols_offset);
          col_ind =
            cols_offset - ((i - max_rows_offset * cols) % max_cols_offset) - 1;
        }
        cell = pu.nx0 * (2 + row_ind) + 2 + col_ind;
      ...
      for (i = cols_offset - 1; i < cols + cols_offset + 5; i += 5) {
        ...
        for (row_ind = 0; row_ind < rows + rows_offset; row_ind++) {
          var edgex =
            pu.nx0 * pu.ny0 + pu.nx0 * (1 + row_ind) + 1 + col_ind + 1;
    ...
    case "easyasabc":
      ...
      pu["pu_q"].number[pu.nx0 * 2 + cols + 2] = [
        `(A-${string_map[number] || number})`,
        1,
        "8",
      ];
